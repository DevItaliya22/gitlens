...
export const FileDiffBlock: React.FC<FileDiffBlockProps> = ({
  file,
  ...
  const renderPatch = (patch: string, filename: string) => {
    if (!patch)
      return (
        <div className="p-4 text-fg-muted text-sm italic">
    ...
    const bottomSpacer = Math.max(0, (lines.length - end) * ROW_HEIGHT);

    return (
      <div
        className="overflow-auto text-xs font-mono leading-5"
      ...
      >
        <table className="w-full border-collapse">
          <tbody>
            {topSpacer > 0 && (
              <tr>
                <td colSpan={2} style={{ height: topSpacer }} />
              </tr>
            )}
              ...
              return (
                ...
                >
                  ...
                  <td className={`pl-2 whitespace-pre ${textClass}`}>
                    {marker && (
                      <span className="select-none opacity-60 mr-1 inline-block w-3">
                        {marker}
                      </span>
                    )}
                    <span
            ...
            {bottomSpacer > 0 && (
              <tr>
                <td colSpan={2} style={{ height: bottomSpacer }} />
              </tr>
            )}
          </tbody>
        </table>
